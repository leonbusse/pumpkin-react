...
import { PumpkinTrack, createPlaylist } from "../../api/pumpkin";
...
export const SharePage: FC = () => {
  ...
  const [activeMobileScreen, setActiveMobileScreen] = useState<MobileScreen>(
    MobileScreen.Listen
  );
  ...
  const { togglePlayback, playing, setPlayling, audioPlayerRef } = usePlayer(
    trackIndex
  );
  ...
  function likeTrack(libraryUserId: string, track: PumpkinTrack) {
    ...
    const previousLikes =
      likes[libraryUserId]?.filter((t) => t.id !== track.id) || [];
  ...
  function removeLikedTracks(libraryUserId: string, tracks: PumpkinTrack[]) {
    ...
    const newLikes = {
      ...likes,
      [libraryUserId]: previousLikes.filter((t) => !tracks.includes(t)),
    };
    globalSetters.setPumpkinState({
      likes: newLikes,
    });
  ...
  const onDelete = (tracks: PumpkinTrack[]) =>
    removeLikedTracks(shareId, tracks);
  ...
  const onButtonDone = useCallback(
    async (closable?: boolean) => {
  ...
  if (window.location.search.includes("action=createPlaylist")) {
    const afterPlaylistSplit =
      window.location.search.split("playlistName=", 2)[1] || "";
    const playlistName = decodeURIComponent(
      afterPlaylistSplit.split("=")[0] || ""
    );
    (async () => {
      onCreatePlaylist(playlistName);
    })();
